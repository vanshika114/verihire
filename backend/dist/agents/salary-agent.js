class SalaryAgent {
    ...
    analyze(metadata) {
        ...
        const paymentScamScore = this.detectPaymentBeforeJoining(extractedText, signals, redFlags);
        score -= paymentScamScore;
        // Check for unrealistic salary
        const salaryScore = this.detectUnrealisticSalary(metadata, extractedText, signals, redFlags);
        score -= salaryScore;
        // Check for impossible benefits
        const benefitsScore = this.detectImpossibleBenefits(extractedText, signals, redFlags);
        score -= benefitsScore;
        // Check for work from home + huge salary mismatch
        const wfhScore = this.detectWorkFromHomeScam(metadata, extractedText, signals, redFlags);
        score -= wfhScore;
        // Check for no experience + very high pay
        const experienceScore = this.detectExperienceMismatch(metadata, extractedText, signals, redFlags);
        score -= experienceScore;
        // Check for joining bonus scams
        const bonusScore = this.detectJoiningBonusScam(metadata, extractedText, signals, redFlags);
        score -= bonusScore;
    ...
    detectPaymentBeforeJoining(text, signals, redFlags) {
        let score = 0;
        ...
        for (const pattern of paymentPatterns) {
            if (pattern.test(text)) {
                signals.push({
                    type: 'PAYMENT_BEFORE_JOINING',
                    severity: 'high',
                    description: 'Text indicates payment required before joining - major red flag',
                });
    ...
    detectUnrealisticSalary(metadata, text, signals, redFlags) {
        let score = 0;
        ...
        if (salary) {
            ...
            if (monthlyEquivalent > this.SALARY_THRESHOLDS.UNREALISTIC_MONTHLY) {
                ...
                redFlags.push(`Unrealistic salary: ₹${monthlyEquivalent.toLocaleString()}/month`);
                score += 35;
            }
    ...
    detectImpossibleBenefits(text, signals, redFlags) {
        let score = 0;
    ...
    detectWorkFromHomeScam(metadata, text, signals, redFlags) {
        let score = 0;
        const isRemote = text.includes('work from home') ||
        ...
        if (isRemote) {
            ...
            if (monthlyEquivalent > this.SALARY_THRESHOLDS.ENTRY_LEVEL_MAX * 0.8 &&
                ...
                if (!this.hasRealisticSkillsRequired(text)) {
                    ...
                    redFlags.push(`Remote job with unusually high salary: ₹${monthlyEquivalent.toLocaleString()}/month`);
                    score += 20;
                }
            }
        }
    ...
    detectExperienceMismatch(metadata, text, signals, redFlags) {
        let score = 0;
        const isEntryLevel = text.includes('fresher') ||
        ...
        if (isEntryLevel) {
            ...
            if (monthlyEquivalent > this.SALARY_THRESHOLDS.ENTRY_LEVEL_MAX) {
                ...
                redFlags.push(`Fresher/Entry-level position offering ₹${monthlyEquivalent.toLocaleString()}/month`);
                score += 35;
            }
        }
        return score;
    }
    detectJoiningBonusScam(metadata, text, signals, redFlags) {
        let score = 0;
        const bonusPatterns = [
            /joining bonus.*₹?[\d,]+/gi,
            /sign.*bonus.*₹?[\d,]+/gi,
    ...
    hasRealisticSkillsRequired(text) {
        const skillKeywords = [
            'experience',
            'technical',
            'proficiency',
