export class RecruiterAgent {
    async analyze(payload) {
        const extractedText = payload.metadata?.extractedText || "";
        const signals = [];
        const redFlags = [];
        let score = 100;
        ...
        const professionalCount = professionalKeywords.filter(kw => extractedText.toLowerCase().includes(kw)).length;
        if (professionalCount >= 2) {
            signals.push("Professional language detected");
            score += 10;
        }
        // Check for urgency indicators
        const urgencyIndicators = unprofessionalKeywords.filter(kw => extractedText.toLowerCase().includes(kw));
