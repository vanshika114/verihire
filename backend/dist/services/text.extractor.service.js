...
export class TextExtractorService {
     ...
     */
    async extract(fileBuffer, fileType) {
        try {
            // Validate buffer
            ...
            if (!detectedType || !['pdf', 'image'].includes(detectedType)) {
                const error = new Error('Unable to determine file type. Ensure file is a valid PDF or image.');
            ...
            if (error instanceof Error) {
                ...
                // Wrap unknown errors
                const extractorError = new Error(`Text extraction failed: ${error.message}`);
                extractorError.name = 'TextExtractionError';
                throw extractorError;
            }
            const unknownError = new Error('An unknown error occurred during text extraction');
    ...
    async extractFromPdf(fileBuffer) {
        try {
            ...
            const extractorError = new Error(`PDF extraction failed: ${errorMessage}`);
    ...
    async extractFromImage(fileBuffer) {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            const extractorError = new Error(`Image extraction failed: ${errorMessage}`);
     ...
     */
    async extractWithFallback(fileBuffer) {
        try {
            ...
            try {
                ...
                const extractorError = new Error(`Text extraction failed with all methods. Primary: ${errorMessage}. Fallback: ${fallbackMessage}`);
