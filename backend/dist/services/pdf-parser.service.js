...
export class PdfParserService {
    ...
    async extract(buffer) {
        try {
            ...
            if (pdfSignature !== '%PDF') {
                const error = new Error('Invalid PDF file: missing PDF file signature. File does not start with %PDF');
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes('PDF file not found') ||
                    ...
                    const parsedError = new Error(`PDF parsing failed: ${error.message}`);
                ...
                if (error.message.includes('EOF') ||
                    ...
                    const corruptError = new Error('PDF file appears to be corrupted or truncated');
                    corruptError.name = 'CorruptedPdfFile';
                    throw corruptError;
                }
                // Generic error handling
                const genericError = new Error(`Failed to extract PDF text: ${error.message}`);
                genericError.name = 'PdfExtractionError';
                throw genericError;
            }
            // Handle unknown error types
            const unknownError = new Error('An unknown error occurred while parsing the PDF');
