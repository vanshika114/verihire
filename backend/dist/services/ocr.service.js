...
export class OcrService {
    ...
    async initialize() {
        try {
            ...
            const initError = new Error(`Failed to initialize OCR worker: ${error instanceof Error ? error.message : 'Unknown error'}`);
     ...
     */
    async extract(imageBuffer, language = 'eng') {
        try {
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes('Worker') || error.message.includes('worker')) {
                    const workerError = new Error(`OCR worker error: ${error.message}`);
                    workerError.name = 'OcrWorkerError';
                    throw workerError;
                }
                // Handle timeout errors
                if (error.message.includes('timeout') || error.message.includes('Timeout')) {
                    const timeoutError = new Error('OCR processing timed out. The image may be too large or complex.');
                    timeoutError.name = 'OcrTimeout';
                    throw timeoutError;
                }
                // Generic error handling
                const genericError = new Error(`Failed to extract text from image: ${error.message}`);
                genericError.name = 'OcrExtractionError';
                throw genericError;
            }
            // Handle unknown error types
            const unknownError = new Error('An unknown error occurred during OCR processing');
    ...
    async terminate() {
        try {
            ...
            console.error('Error terminating OCR worker:', error instanceof Error ? error.message : 'Unknown error');
        }
    }
    /**
