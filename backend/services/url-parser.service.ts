...
export interface UrlParserError extends Error {
  name: string;
  message: string;
  code?: string;
}
...
export class UrlParserService {
  private static readonly VALID_PROTOCOLS = ['http:', 'https:', 'ftp:', 'ftps:'];
    ...
    try {
      ...
      if (trimmedUrl.length > UrlParserService.MAX_URL_LENGTH) {
        const error = new Error(
          `URL exceeds maximum length of ${UrlParserService.MAX_URL_LENGTH} characters`
        );
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('Invalid URL')) {
          const parseError = new Error(
            `Failed to parse URL: ${urlString}. The URL format is invalid.`
          );
      ...
      const unknownError = new Error(
        'An unknown error occurred while parsing the URL'
      );
   ...
   */
  isValid(urlString: string, allowedProtocols: string[] = ['http:', 'https:']): boolean {
    try {
   ...
   * @throws UrlParserError if URL is invalid
   */
  getDomain(urlString: string): string {
    try {
      const parsed = this.parse(urlString);
   ...
   */
  private hasProtocol(urlString: string): boolean {
    return UrlParserService.VALID_PROTOCOLS.some(
      protocol => urlString.toLowerCase().startsWith(protocol)
    );
  }
   ...
   */
  isHostnameSafe(hostname: string): boolean {
    try {
