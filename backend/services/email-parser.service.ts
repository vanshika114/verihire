...
export interface EmailParserError extends Error {
  name: string;
  message: string;
  code?: string;
}
...
export class EmailParserService {
  ...
  private static readonly STRICT_EMAIL_REGEX = /^(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9]))\.){3}(?:(2(5[0-5]|[0-4][0-9])|1[0-9][0-9]|[1-9]?[0-9])|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])$/i;
   ...
   */
  parse(emailString: string, strict: boolean = false): ParsedEmail {
    try {
      // Validate input
      ...
      if (trimmedEmail.length > EmailParserService.MAX_EMAIL_LENGTH) {
        const error = new Error(
          `Email exceeds maximum length of ${EmailParserService.MAX_EMAIL_LENGTH} characters`
        );
      ...
      if (atCount !== 1) {
        const error = new Error(
          `Email must contain exactly one '@' symbol, found ${atCount}`
        );
      ...
      if (localPart.length > EmailParserService.MAX_LOCAL_PART_LENGTH) {
        const error = new Error(
          `Email local part exceeds maximum length of ${EmailParserService.MAX_LOCAL_PART_LENGTH} characters`
        );
      ...
      if (domain.length > EmailParserService.MAX_DOMAIN_LENGTH) {
        const error = new Error(
          `Email domain exceeds maximum length of ${EmailParserService.MAX_DOMAIN_LENGTH} characters`
        );
      ...
      if (!regex.test(trimmedEmail)) {
        const error = new Error(
          `Email format is invalid: ${trimmedEmail}`
        );
      ...
      const unknownError = new Error(
        'An unknown error occurred while parsing the email'
      );
   ...
   */
  isValid(emailString: string, strict: boolean = false): boolean {
    try {
   ...
   */
  normalize(emailString: string): string {
    try {
   ...
   * @throws EmailParserError if email is invalid
   */
  getLocalPart(emailString: string): string {
    try {
      const parsed = this.parse(emailString);
   ...
   * @throws EmailParserError if email is invalid
   */
  getDomain(emailString: string): string {
    try {
      const parsed = this.parse(emailString);
   ...
   */
  areEqual(email1: string, email2: string): boolean {
    try {
   ...
   */
  isDomainSafe(emailString: string): boolean {
    try {
      const parsed = this.parse(emailString);
   ...
   * @throws EmailParserError if email is invalid
   */
  maskEmail(emailString: string): string {
    try {
      const parsed = this.parse(emailString);
