...
export class AuthenticityAgent {
  private readonly AUTHENTICITY_WEIGHTS = {
    documentAuthenticity: 0.4,
    sourceCredibility: 0.35,
    contentQuality: 0.25,
  };

  private readonly PAYMENT_KEYWORDS = [
    "registration fee",
    "processing fee",
    "training fee",
    "security deposit",
  ...
  private readonly UNREALISTIC_SALARY_PATTERNS = [
    "₹1,00,000+",
    "₹2,00,000+",
    "lakh per month",
    "2 lakh",
  ...
  private readonly SUSPICIOUS_WORDING = [
    "guaranteed",
  ...
  private readonly KNOWN_COMPANIES = [
    "google",
    "microsoft",
    "amazon",
    "apple",
  ...
  private readonly FAKE_OFFER_INDICATORS = [
  ...
  async analyze(input: {
    ...
    try {
      ...
      analysis.details.documentAuthenticity = Math.max(
        0,
        Math.min(100, analysis.details.documentAuthenticity)
      );
      analysis.details.sourceCredibility = Math.max(
        0,
        Math.min(100, analysis.details.sourceCredibility)
      );
      analysis.details.contentQuality = Math.max(
        0,
        Math.min(100, analysis.details.contentQuality)
      );
  ...
  private extractText(metadata?: Record<string, unknown>): string {
    if (!metadata) return "";
  ...
  private analyzeJobPostingContent(
    analysis: AuthenticityAnalysis,
    text: string,
    metadata?: Record<string, unknown>
  ): void {
    ...
    const hasSalary =
      text.includes("salary") || text.includes("₹") || text.includes("$");
    const hasLocation =
      text.includes("location") || text.includes("city") || text.includes("office");
    ...
    const fakeOfferCount = this.FAKE_OFFER_INDICATORS.filter((indicator) =>
  ...
  private detectPaymentRequests(
    analysis: AuthenticityAnalysis,
    text: string
  ): void {
    const paymentMatches = this.PAYMENT_KEYWORDS.filter((keyword) =>
      text.includes(keyword)
    );

    if (paymentMatches.length > 0) {
      analysis.signals.suspicious.push(
        `Mentions payment: ${paymentMatches.join(", ")}`
      );
  ...
  private detectUrgencyLanguage(
    ...
    const urgentMatches = this.URGENT_KEYWORDS.filter((keyword) =>
      text.includes(keyword)
    );
  ...
  private detectPhishingPatterns(
    ...
    const phishingMatches = this.PHISHING_KEYWORDS.filter((keyword) =>
      text.includes(keyword)
    );
    ...
    if (phishingMatches.length >= 2) {
      ...
      analysis.signals.suspicious.push(
        `Potential phishing pattern: ${phishingMatches[0]}`
      );
      analysis.details.sourceCredibility -= 20;
    }
  }

  private detectSuspiciousWording(
    analysis: AuthenticityAnalysis,
    text: string
  ): void {
    const suspiciousMatches = this.SUSPICIOUS_WORDING.filter((keyword) =>
      text.includes(keyword)
    );

    if (suspiciousMatches.length >= 3) {
      analysis.signals.suspicious.push(
        "Multiple suspicious phrases detected"
      );
      analysis.details.contentQuality -= 35;
      analysis.redFlags.push("Suspicious job offer language");
    } else if (suspiciousMatches.length >= 1) {
      analysis.signals.suspicious.push(
        `Contains suspicious phrase: ${suspiciousMatches[0]}`
      );
      analysis.details.contentQuality -= 15;
    }
  }

  private detectContactIssues(
    analysis: AuthenticityAnalysis,
    text: string
  ): void {
    ...
    if (!hasFormalContact && (hasWhatsapp || hasTelegram)) {
      analysis.signals.suspicious.push(
        "Only WhatsApp/Telegram contact, no email/phone"
      );
      analysis.details.sourceCredibility -= 35;
      analysis.redFlags.push(
        "Only informal contact methods provided"
      );
  ...
  private detectGrammarQuality(
    ...
    const content =
      ...
      (metadata?.extractedText as string) ||
      "";
    if (content) {
  ...
  private detectFakeOffers(
    analysis: AuthenticityAnalysis,
    text: string
  ): void {
    const fakeIndicators = this.FAKE_OFFER_INDICATORS.filter((indicator) =>
      text.includes(indicator)
    );
  ...
  private hasSuspiciousDomain(domain: string): boolean {
  ...
  private analyzeGrammarIssues(text: string): number {
    let issues = 0;

    // Check for common grammar patterns
    if (/u r |ur [a-z]/.test(text)) issues++; // "u r" instead of "you are"
    if (/\b(teh|recieved|occured|bussiness)\b/i.test(text)) issues++; // Common misspellings
    if (/[!]{2,}|[\?]{2,}/.test(text)) issues++; // Multiple exclamation/question marks
    if (/\b([a-z])\1{2,}\b/i.test(text)) issues++; // Repeated characters
    if (!/[.!?]$/.test(text.trim())) issues++; // No ending punctuation

    return issues;
  }

  private hasUnrealisticSalary(text: string): boolean {
    const textLower = text.toLowerCase();
    return this.UNREALISTIC_SALARY_PATTERNS.some((pattern) =>
  ...
  private isKnownCompany(company: string): boolean {
    ...
    return this.KNOWN_COMPANIES.some(
      (known) =>
        companyLower.includes(known) || known.includes(companyLower)
    );
  }
  ...
  private isOfficialDomain(domain: string): boolean {
    ...
    return knownOfficialDomains.some((official) =>
      domainLower.endsWith(official)
    );
  }

  private calculateWeightedScore(details: {
    documentAuthenticity: number;
    sourceCredibility: number;
    contentQuality: number;
  }): number {
    const rawScore =
      details.documentAuthenticity *
